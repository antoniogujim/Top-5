...
export default function CreateRanking() {
  ...
  const { addRanking, updateRanking, rankings, canCreate, refetch } = useRankings()
  ...
  const handleRemoveCategory = (value: string) => {
    setPendingDeleteCat(value)
  }

  const confirmDeleteCategory = async () => {
    if (!pendingDeleteCat) return
    await removeCategory(pendingDeleteCat)
    if (category === pendingDeleteCat) setCategory(categories.find((c) => c.value !== pendingDeleteCat)?.value ?? '')
    refetch()
  ...
  return (
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {isPremium ? (
              ...
              <button
                ...
                className="px-2.5 py-1 rounded-full border border-yellow-300 dark:border-yellow-600 text-yellow-600 dark:text-yellow-400 text-xs font-semibold hover:bg-yellow-50 dark:hover:bg-yellow-950"
              >
                Premium
              </button>
            )}
    ...
    {pendingDeleteCat && (
      <Modal
        ...
        confirmDanger
      >
        Se eliminarán también todos tus rankings con esta categoría. ¿Continuar?
      </Modal>
    )}
    </>
  )
